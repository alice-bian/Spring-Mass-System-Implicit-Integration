...
public:
    ...
    TV evaluateSpringForce(int idx, std::vector<TV>& dx) {
        // Evaluate fA_spring only, i.e., The spring force on the first node of the segment.
        ...
        Eigen::Matrix<int,2,1> segment = segments[idx];
        TV x_1 = TV::Zero();
        x_1 = x[segment[0]] + dx[segment[0]];
        TV x_2 = TV::Zero();
        x_2 = x[segment[1]] + dx[segment[1]];
        T l = std::sqrt(std::pow(x_1[0] - x_2[0], 2) + std::pow(x_1[1] - x_2[1], 2) + std::pow(x_1[2] - x_2[2], 2));
        T l_0 = rest_length[idx];
        TV n_12 = TV::Zero();
        n_12 = (x_1 - x_2) / l;
        TV f_1 = TV::Zero();
    ...
    TV evaluateDampingForce(int idx, std::vector<TV>& dx, T dt) {
        // Evaluate fA_damping only, i.e., The damping force on the first node of the segment.
    ...
    TM evaluateKS(int idx, std::vector<TV>& dx, bool project_pd=true) {
        // Evaluate dfA_spring/dxA and store it in KS,
        ...
        TM I = TM::Identity();
        T l_0 = rest_length[idx];
        KS = youngs_modulus * ((1.f / l_0) - (1.f / l)) * (I - (n_12 * n_12.transpose())) + (youngs_modulus / l_0) * n_12 * n_12.transpose();
        KS *= (-1);
    ...
    TM evaluateKD(int idx, T dt) {
        // Evaluate dfA_damping/dxA and store it in KS,
        //       i.e. the derivaitive of the spring force on the first node of the segment w.r.t the position of the first node.
        ...
        x_2 = x[segment[1]];
        TV n_12 = TV::Zero();
        n_12 = (x_1 - x_2).normalized();
        KD = (-1) * damping_coeff * n_12 * n_12.transpose() / dt;
        return KD;
    }
